...
def replicate_unitcell(lattice, positions, num_atoms, numbers, reps):
    ...
    orig_numbers = np.array(numbers, dtype=int)
    new_positions = []
    new_numbers = []
    ...
    for ix in range(nx):
        for iy in range(ny):
            for iz in range(nz):
                shift = np.array([ix * lattice[0][0],
                                  iy * lattice[1][1],
                                  iz * lattice[2][2]])
                for pos, num in zip(orig_positions, orig_numbers):
                    new_positions.append(pos + shift)
                    new_numbers.append(num)

    positions = np.array(new_positions, dtype=float)
    numbers = np.array(new_numbers, dtype=int)

    lattice = np.array([
        [lattice[0][0] * nx, lattice[0][1], lattice[0][2]],
        [lattice[1][0], lattice[1][1] * ny, lattice[1][2]],
        [lattice[2][0], lattice[2][1], lattice[2][2] * nz]
    ], dtype=float)

    num_atoms = [n * nx * ny * nz for n in num_atoms]

    return lattice, positions, num_atoms, numbers
...
def write_poscar( outName,H,ntypes,species,positions,tag ):

    out = open(outName,'w')
    out.write(f"Diamond Surface {tag}\n")
    out.write("1.00000" + '\n')
    out.write("%5f %5f %5f" % (H[0][0],H[0][1],H[0][2]) + '\n')
    out.write("%5f %5f %5f" % (H[1][0],H[1][1],H[1][2]) + '\n')
    out.write("%5f %5f %5f" % (H[2][0],H[2][1],H[2][2]) + '\n')
    for s in unique_items(species):
        out.write("%s " % s)
    out.write('\n')
    for type in ntypes:
        out.write("%i " % type)
